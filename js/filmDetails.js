$(function(){
	...
	for(var i=0;i<data.length;i++){
		var filmName = data[i].imgName;
		var year = data[i].releaseDate.slice(0,4);
		//加载对应电影详细信息
		if(getRequestMsg.filmName==filmName&&getRequestMsg.releaseDate==year){
			/*摘要*/
			$(".filmAbstract h1").text(data[i].textName);
			if(data[i].textName==""){
				$(".filmAbstract h1").text(filmName);
			}
			...
			/*电影截图*/
			for(var m in data[i].carousel){
				var carousel = data[i].carousel[m];
				$(".carousel ul").append("<li><img src='"+carousel+"' ></li>");
				$(".thumbnail ul").append("<li><img src='"+carousel+"' ></li>");
			}
			if(data[i].carousel.length>0){
