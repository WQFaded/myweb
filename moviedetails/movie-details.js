$(function(){
	...
	if($.isEmptyObject(movieMsg)){ //判断是否为空,为空则显示页面未找到
		$(".container-full").html(
			"<i class='layui-icon layui-icon-404'></i>"
			+"<p>页面未找到，<strong>3</strong> 秒后将返回上一个页面 或</p>"
		...
		var setSecond = setInterval(function(){
			time--;
			$(".container-full p strong").text(time);
			if(time==0){
	...
	$.ajax({
		...
		success: function(data){
			...
			$("title").text(data.movieTitle+"-影品网");
			if(data.movieTitle==""){
			...
			if(data.magnets.length>0){
				for(var k in data.magnets){
					...
					$("#magnetic-link").append("<li><a class='magnet' href='"+magnets.magnet+"'>"+magnets.magnetName+"</a></li>");
